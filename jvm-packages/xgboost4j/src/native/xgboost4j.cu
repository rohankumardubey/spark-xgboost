...
void CopyInterface(std::vector<xgboost::ArrayInterface> const &interface_arr,
  ...
  for (size_t c = 0; c < interface_arr.size(); ++c) {
    ...
    size_t element_size = interface.type[2] - '0';
 ...
 public:
  ...
  int32_t PullIterFromJVM() {
    ...
    if (jenv_->CallBooleanMethod(jiter_, has_next)) {
      ...
      jmethodID getCudfJsonStr = CheckJvmCall(jenv_->GetMethodID(
          batch_class, "getCudfJsonStr", "()Ljava/lang/String;"), jenv_);

      auto jinterface =
          static_cast<jstring>(jenv_->CallObjectMethod(batch, getCudfJsonStr));
  ...
  void StageData(std::string interface_str) {
    ...
    XGDeviceQuantileDMatrixSetDataCudaColumnar(proxy_,
                                               interface_str.c_str());
    it_++;
  }
  ...
  int NextSecondLoop() {
    ...
    XGDeviceQuantileDMatrixSetDataCudaColumnar(proxy_, interface_str.c_str());
    it_++;
    return 1;
  }
