...
import ml.dmlc.xgboost4j.scala.{Booster, ColumnDMatrix, DMatrix, EvalTrait, ExternalCheckpointManager, ObjectiveTrait}
...
private[spark] object GpuXGBoost {
  ...
  // ====== build watches ======
  private def buildDMatrixIncrementally(
      iter: Iterator[GpuColumnBatch],
      indices: ColumnIndices,
      missing: Float,
      inferNumClass: Boolean,
      maxBin: Int): (DMatrix, Double) = {
    var dm: ColumnDMatrix = null
    var max: Double = Double.MinValue
    // For LTR
    val groupInfo = new mutable.ArrayBuffer[java.lang.Integer]
    val weightInfo = new mutable.ArrayBuffer[java.lang.Float]
    var groupId = 0
    val isLTR = false // Disable LTR now

    while (iter.hasNext) {
      val colBatch = iter.next
      if (isLTR) {
        // Build group info, along with weight info if needed (-1 means no weight)
        val weightIdx = indices.weightId.getOrElse(-1)
        groupId = colBatch.groupAndAggregateOnColumnsHost(indices.groupId.get, weightIdx, groupId,
          groupInfo.asJava, weightInfo.asJava)
      }

      // Build DMatrix
      val featureStr = colBatch.getArrayInterface(indices.featureIds: _*)
      val labelStr = colBatch.getArrayInterface(indices.labelId)
      val weightStr = indices.weightId.map(wi => colBatch.getArrayInterface(wi))
      val marginStr = indices.marginId.map(mi => colBatch.getArrayInterface(mi))

      if (dm == null) {
        dm = new ColumnDMatrix(featureStr, missing, 1)
        dm.setLabel(labelStr)
        weightStr.foreach(dm.setWeight)
        marginStr.foreach(dm.setBaseMargin)
      } else {
        // TODO Pending by native
        logger.warn("Incremental building DMatrix is not supported now !")
      }

      if (inferNumClass) {
        // calculate max label to reduce
        val tmpMax = colBatch.getMaxInColumn(indices.labelId)
        max = if (tmpMax > max) tmpMax else max
      }
      colBatch.close()
    }
    if (dm != null && isLTR) {
      ...
      if (weightInfo.nonEmpty) dm.setWeight(weightInfo.map(_.floatValue).toArray)
    }
    (dm, max)
  }

  private[this] class RapidsIterator(base: Iterator[GpuColumnBatch], inferNumClass: Boolean,
      indices: ColumnIndices) extends Iterator[CudfTable] {
    ...
    override def next(): CudfTable = {
      ...
      new CudfTable(gpuColumnBatch.getTable(),
        seqIntToSeqInteger(indices.featureIds).asJava,
        seqIntToSeqInteger(Seq(indices.labelId)).asJava,
        seqIntToSeqInteger(weights).asJava,
        seqIntToSeqInteger(margins).asJava)
    }
  }
  ...
  private def buildDMatrix(
    ...
    if (true) {
      ...
      (new ColumnDMatrix(rapidsIterator, missing, maxBin, 1), rapidsIterator.maxLabels)
    } else {
      ...
      // Build DMatrix
      val featureStr = singleColBatch.getArrayInterface(indices.featureIds: _*)
      val labelStr = singleColBatch.getArrayInterface(indices.labelId)
      val weightStr = indices.weightId.map(wi => singleColBatch.getArrayInterface(wi))
      val marginStr = indices.marginId.map(mi => singleColBatch.getArrayInterface(mi))

      val dm = new ColumnDMatrix(featureStr, missing, 1)
      dm.setLabel(labelStr)
      weightStr.foreach(dm.setWeight)
      marginStr.foreach(dm.setBaseMargin)
